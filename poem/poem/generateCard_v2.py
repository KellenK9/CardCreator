...
temp_dict_equipment = {
    ...
    "attack": "2", #Only matters if type is sword
    "color": "red",
...
def generate_equipment_card(dict):
    ...
    # Import and color frame
    if(dict["type"] == "Sword"):
        frame = Image.open("card_frames/Sword_frame.png")
    else:
        frame = Image.open("card_frames/Shield_frame.png")
    data = np.array(frame)
    red, green, blue, alpha = data.T
    ...
    attack_txt = Image.new("RGBA", frame.size, (255, 255, 255, 0))
    fnt1 = ImageFont.truetype(current_font, name_font_size)
    ...
    name_obj = ImageDraw.Draw(name_txt)
    description_obj = ImageDraw.Draw(description_txt)
    attack_obj = ImageDraw.Draw(attack_txt)
    ...
    attack_obj.text((stamina_x, stamina_y), dict["attack"], font=fnt3, fill=font_color_nums)
